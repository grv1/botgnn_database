...
def generateH(modes, target, db, seed_table, gamma, phi, depth_bound, length_bound, col_dict, bottom_clause):
    init_hyp = Hypothesis([target],seed_table,0,target.min_var, target.max_var, 0)
    bottom_clause.insert(init_hyp)
    
    openset = [init_hyp]
    cur_table_index = [1]
    
    while (openset != []):
        cur_hyp = gamma(openset)
...
#                    cur_extension.output()
                    ...
                    for next_hyp in next_hyps:
                        if phi(next_hyp.t):
                            bottom_clause.insert(next_hyp)
                            if (next_hyp.depth <= depth_bound):
                                openset.append(next_hyp)
...
#    scratch_db = pg.connect(dbname='train_scratch')
    ...
    modes = []
    modes.append(Mode(1,'has_car',2,['input','output']))
    modes.append(Mode(-1,'long',1,['input']))
    modes.append(Mode(1,'short',1,['input']))
    modes.append(Mode(1,'open',1,['input']))
    modes.append(Mode(1,'closed',1,['input']))
    modes.append(Mode(1,'double',1,['input']))
    modes.append(Mode(1,'jagged',1,['input']))
 ...
 #   modes.append(Mode(1,'load',3,['input','const','const']))
  #  modes.append(Mode(1,'wheels',2,['input','const']))
   # modes.append(Mode(1,'infront',2,['input','input']))
    ...
    target = Predicate('eastbound', 1, 0, [1], 1, 1, None)

    # set up seed table #
    selection_clause = 'where train=\'east1\' or train=\'east5\''
#    selection_clause = ''
    seed_table = create_seed(back_db, 'eastbound', 1, ['train'], selection_clause)
    ...
    #phi = dummy_filter
    threshold = 1
    phi = init_filter(back_db, threshold, seed_table)

    # set up bottom clause #
    bc = BC_Printer()
    
    # run generateH #
    generateH(modes, target, back_db, seed_table, gamma, phi, 2, 12, columns, bc)
